...
local function AddColor(ent)
    ent:DoTaskInTime(GLOBAL.FRAMES, function()
        if ent and ent.AnimState then
            ent.AnimState:SetMultColour(1, 0, 0, 1)
            ent:AddTag("pc_no_pickup")
            print("Added tag to ", ent.prefab or "unknown")
        else
            print("Failed to add tag to ", ent.prefab or "unknown")
        end
    end)
end
...
local ItemRep = require("components/inventoryitem_replica")
if not ItemRep.__pcfilter_patched then
    ItemRep.__pcfilter_patched = true
    local old = ItemRep.CanBePickedUp

    function ItemRep:CanBePickedUp(picker)
        if picker == GLOBAL.ThePlayer and self.inst:HasTag("pc_no_pickup") then
            return false
        end
